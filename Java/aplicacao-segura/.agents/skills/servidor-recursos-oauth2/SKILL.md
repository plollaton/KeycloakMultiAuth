...
3. Se o caminho é protegido (`GET /api/protected`), a cadeia exige um
   ...
   regra 6: assinatura contra o JWKS do Keycloak, `iss`, `exp` e `aud`.
...
ao menos as claims `iss`, `exp` e `aud` (regra 6); o formato completo do
...
- As três verificações da regra 6 (assinatura, `iss`/`exp`/`aud`) são
