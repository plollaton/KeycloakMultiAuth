...
description: >
  ...
  claims iss/exp/aud) na SecurityFilterChain. Usar ao implementar, alterar ou
...
serve a um propósito diferente: validar o `client_assertion` que esta
aplicação envia ao Keycloak quando atua como client).

A aplicação acumula, no mesmo processo, o papel de OAuth2 Client (domínio
Cliente OAuth2 com Client Assertion JWT) e o papel de OAuth2 Resource Server
coberto por este domínio. Os dois papéis são independentes: este domínio não
gera nem envia tokens, apenas recebe e valida `access_tokens` emitidos pelo
Keycloak para outras aplicações que chamam esta aplicação.
...
6. `GET /oauth2/jwks` — o endpoint de publicação da chave pública desta
...
7. Para considerar um `access_token` válido em um endpoint protegido, a
   aplicação valida:
   ...
   negócio disponível; a exigência de negócio fixada é que essas três
   verificações (assinatura, `iss`, `exp`, `aud`) ocorram para todo
   `access_token` apresentado a um endpoint protegido.
8. Um `access_token` que falhe em qualquer uma das verificações da regra 7
   (assinatura inválida, emissor incorreto, token expirado ou audiência
   incorreta) — ou a ausência de um `access_token` na requisição — impede o
   processamento de um endpoint protegido: a requisição é rejeitada antes de
   alcançar a lógica de negócio do endpoint.
9. A classificação de público/protegido e as verificações da regra 7 se
...
1. A cadeia identifica o caminho requisitado e o classifica como público ou
   protegido (regras 1 a 6).
2. Se o caminho é público (`GET /api/public`, `GET /api/cross`,
   `GET /actuator/health`, `GET /oauth2/jwks`, e os caminhos do
   Swagger/OpenAPI liberados pela convenção de documentação de API), a
   requisição é processada sem exigir `access_token`.
3. Se o caminho é protegido (`GET /api/protected`), a cadeia exige um
   ...
   regra 7: assinatura contra o JWKS do Keycloak, `iss`, `exp` e `aud`.
...
Este fluxo independe do fluxo de obtenção de `access_token` do domínio
Cliente OAuth2 com Client Assertion JWT: os tokens validados aqui são
emitidos pelo Keycloak para quem quer que chame esta aplicação como resource
server — podendo inclusive ter sido obtidos por esta mesma aplicação atuando
como client em outro momento, caso ela chame a si mesma, ou por qualquer
outro chamador autorizado pelo Keycloak.
...
ao menos as claims `iss`, `exp` e `aud` (regra 7); o formato completo do
token e demais claims que o Keycloak inclua são um contrato do Keycloak, não
deste domínio.
...
- As três verificações da regra 7 (assinatura, `iss`/`exp`/`aud`) são
